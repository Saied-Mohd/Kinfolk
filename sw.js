const VERSION='kinfolk-v58';
const SHELL=['./','./index.html','./manifest.webmanifest','./icon-192.png','./icon-512.png'];
self.addEventListener('install',e=>{e.waitUntil(caches.open(VERSION).then(c=>c.addAll(SHELL)).then(()=>self.skipWaiting()))});
self.addEventListener('activate',e=>{e.waitUntil(caches.keys().then(ks=>Promise.all(ks.filter(k=>k!==VERSION).map(k=>caches.delete(k)))).then(()=>self.clients.claim()))});
self.addEventListener('fetch',e=>{const u=new URL(e.request.url);
  if(e.request.method!=='GET')return;
  if(/openrouter\.ai|elevenlabs\.io|api\.openai\.com|artificialanalysis\.ai|workers\.dev|trycloudflare\.com|fal\.run|fal\.media|queue\.fal\.run|rest\.alpha\.fal\.ai/.test(u.host))return;               // never cache API calls
